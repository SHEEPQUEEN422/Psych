...
```{r}
 mean_rating<- tea_data |>
   group_by(condition) |>
   summarize(mean_rating=mean(rating))
 tea_data<-tea_data|>
   left_join(mean_rating,by="condition")
```
...
false_positive_rate<-mean(all_results$pvalue<.05)
false_positive_rate
```
...
false_positive_rate<-mean(all_results_high_n$pvalue<.05)
false_positive_rate
```
